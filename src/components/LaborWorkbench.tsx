...
import type { LaborEntry, LaborProjectMeta, LaborProjectRecord, LaborWorker } from '../laborTypes';
import {
...
type LaborTab = 'company' | 'workers' | 'entries' | 'ledger' | 'report';
...
export default function LaborWorkbench() {
  const [activeTab, setActiveTab] = useState<LaborTab>('company');
  ...
  const reportRows = useMemo(
    () => validWorkers.map((worker) => {
      ...
      if (!meta.workplaceManagementNumber.trim()) missingFields.push('사업장관리번호');
      if (!meta.businessRegistrationNumber.trim()) missingFields.push('사업자등록번호');
      if (!meta.companyName.trim()) missingFields.push('회사명');
      if (!meta.siteName.trim()) missingFields.push('현장명');
      if (!meta.managerName.trim()) missingFields.push('현장책임자');
      if (!meta.companyAddress.trim()) missingFields.push('회사주소');
      if (!meta.companyPhone.trim()) missingFields.push('회사전화번호');
      if (!meta.representativeName.trim()) missingFields.push('대표자명');
      if (!meta.paymentDate.trim()) missingFields.push('지급일');
      return {
        worker,
    ...
    [entries, ledgerRows, meta.businessRegistrationNumber, meta.companyAddress, meta.companyName, meta.companyPhone, meta.paymentDate, meta.representativeName, meta.siteName, meta.workplaceManagementNumber, validWorkers],
  );
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        setNotice({ tone: 'error', message: '노무비 프로젝트 목록을 불러오지 못했습니다.' });
      } finally {
        setLoading(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const pdfBytes = await generateLaborReportPdf({ meta, reportRows });
  ...
  }, [activeTab, meta, reportReady, reportRows]);
      ...
      [field]:
        field === 'workYear' || field === 'workMonth'
          ...
          : field === 'businessRegistrationNumber'
            ? normalizeBusinessNumber(value)
            : field === 'companyPhone'
              ? normalizePhone(value)
              : field === 'workplaceManagementNumber'
                ? normalizeWorkplaceManagementNumber(value)
                : value,
    }));
  };
  ...
  const validateBeforeSave = () => {
    ...
    if (!meta.companyName.trim()) return '회사명을 입력해 주세요.';
    if (!meta.siteName.trim()) return '현장명을 입력해 주세요.';
    if (!meta.workplaceManagementNumber.trim()) return '사업장관리번호를 입력해 주세요.';
    if (!meta.businessRegistrationNumber.trim()) return '사업자등록번호를 입력해 주세요.';
    if (!meta.managerName.trim()) return '현장책임자를 입력해 주세요.';
    if (!meta.representativeName.trim()) return '대표자명을 입력해 주세요.';
    if (!meta.companyPhone.trim()) return '회사 전화번호를 입력해 주세요.';
    if (!meta.companyAddress.trim()) return '회사 주소를 입력해 주세요.';
    if (!meta.paymentDate.trim()) return '지급일을 입력해 주세요.';
    if (!isBusinessNumberFormat(meta.businessRegistrationNumber.trim())) return '사업자등록번호 형식은 000-00-00000 이어야 합니다.';
    if (meta.companyPhone.trim() && !isPhoneFormat(meta.companyPhone.trim())) return '회사 전화번호 형식을 확인해 주세요.';
  ...
  const handleCreateProject = async () => {
    ...
    if (!name?.trim()) return;
    try {
      const created = await createLaborProject(name.trim());
  ...
  const handleSave = async () => {
    ...
    try {
      const nextProject: LaborProjectRecord = {
        ...
        workplaceManagementNumber: meta.workplaceManagementNumber.trim(),
        businessRegistrationNumber: meta.businessRegistrationNumber.trim(),
        companyAddress: meta.companyAddress.trim(),
        companyPhone: meta.companyPhone.trim(),
        representativeName: meta.representativeName.trim(),
      };
      const cleanedWorkers = workers.filter((worker) => worker.name.trim() !== '').map((worker) => ({ ...worker, name: worker.name.trim(), residentId: worker.residentId.trim(), phone: worker.phone.trim(), address: worker.address.trim(), jobType: worker.jobType.trim(), teamName: worker.teamName.trim(), bankName: worker.bankName.trim(), accountNumber: worker.accountNumber.trim(), accountHolder: worker.accountHolder.trim() }));
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto">{(['company', 'workers', 'entries', 'ledger', 'report'] as LaborTab[]).map((tab) => <button key={tab} className={tabClass(tab)} onClick={() => setActiveTab(tab)}>{{ company: '회사정보', workers: '근로자', entries: '노무입력', ledger: '노무비대장', report: '신고서' }[tab]}</button>)}</div>
      </section>
      ...
      {!loading && activeTab === 'company' && (
        <section className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
            ...
            <LabeledInput label="사업장관리번호" value={meta.workplaceManagementNumber} onChange={(value) => setMetaField('workplaceManagementNumber', value)} disabled={!selectedProject} required />
            <LabeledInput label="사업자등록번호" value={meta.businessRegistrationNumber} onChange={(value) => setMetaField('businessRegistrationNumber', value)} disabled={!selectedProject} required />
            <LabeledInput label="대표자명" value={meta.representativeName} onChange={(value) => setMetaField('representativeName', value)} disabled={!selectedProject} required />
            <LabeledInput label="회사 전화번호" value={meta.companyPhone} onChange={(value) => setMetaField('companyPhone', value)} disabled={!selectedProject} required />
            <LabeledInput label="휴대전화" value={meta.companyPhoneMobile} onChange={(value) => setMetaField('companyPhoneMobile', value)} disabled={!selectedProject} />
            <LabeledInput label="팩스번호" value={meta.companyFax} onChange={(value) => setMetaField('companyFax', value)} disabled={!selectedProject} />
            ...
            <div className="md:col-span-2 xl:col-span-3"><LabeledInput label="회사 주소" value={meta.companyAddress} onChange={(value) => setMetaField('companyAddress', value)} disabled={!selectedProject} required /></div>
          </div>
        </section>
      )}
      ...
      {showCalibrator && (
        <div className="fixed inset-0 z-50 bg-gray-900">
          <button
            ...
            className="absolute top-2 right-4 z-50 text-white text-sm bg-red-600 px-3 py-1 rounded"
          >
            닫기
          </button>
          <LaborCalibrator />
      ...
      <LaborPrintSheets printMode={printMode} meta={meta} ledgerRows={ledgerRows} totals={totals} reportRows={reportRows} entries={entries} />
    </div>
  );
}
