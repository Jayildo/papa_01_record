...
- Supabase (DB + Auth 공유 계정 + Edge Function `pin-login`)
...
- PIN 검증은 Edge Function `pin-login` (Deno, `supabase/functions/pin-login/index.ts`) 에서 수행
- 성공 시 공유 계정(`OPERATOR_EMAIL/PASSWORD`)으로 `signInWithPassword` → 세션 토큰 반환 → 클라이언트 `supabase.auth.setSession()` 주입
- 모든 테이블 RLS 활성화 (마이그레이션 018). `authenticated` 만 전체 CRUD, `anon` 권한 회수
- 클라이언트 gate: `supabase.auth.getSession()` + `onAuthStateChange` (App.tsx)
- Edge Function Secrets: `APP_PIN_HASH`(SHA-256), `OPERATOR_EMAIL`, `OPERATOR_PASSWORD`
- `supabase/config.toml` 의 `[functions.pin-login]` 블록에 `verify_jwt = false`
